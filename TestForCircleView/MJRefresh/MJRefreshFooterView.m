...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        // Modified by Lgc, 原来的MJRefresh并没有考虑到国际化的问题，这里手动添加
        self.pullToRefreshText = NSLocalizedString(@"上拉可以加载更多数据", nil);
        self.releaseToRefreshText = NSLocalizedString(@"松开立即加载更多数据", nil);
        self.refreshingText = [NSString stringWithFormat:@"%@...", NSLocalizedString(@"正在刷新", nil)];
    }
    return self;
}
